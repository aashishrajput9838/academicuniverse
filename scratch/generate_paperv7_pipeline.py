...
v7_docx_path = workspace / "docs" / "paper" / "PaperV7_Ollama_Primary.docx"
v7_pdf_path = workspace / "docs" / "paper" / "PaperV7_Ollama_Primary.pdf"

print("============================================================")
print(" GENERATING PAPER V7 (SECTION 6 REMOVED & V7 COMPILED)")
print("============================================================")
...
# -------------------------------------------------------------
# 5.1 REMOVE SECTION 6 (DISCUSSION & THREATS TO VALIDITY) & RENUMBER
# -------------------------------------------------------------
sec6_disc_idx = None
sec7_lim_idx = None
for i, p in enumerate(doc.paragraphs):
    ...
    if (txt.startswith("6. Discussion") or txt.startswith("6. Threats")) and sec6_disc_idx is None:
        sec6_disc_idx = i
    elif (txt.startswith("7. Limitations") or txt.startswith("6. Limitations")) and sec7_lim_idx is None:
        sec7_lim_idx = i
        break

if sec6_disc_idx is not None and sec7_lim_idx is not None:
    disc_paras_to_remove = [doc.paragraphs[k] for k in range(sec6_disc_idx, sec7_lim_idx)]
    for p_del in disc_paras_to_remove:
        delete_paragraph(p_del)

# Renumber remaining sections: 7 -> 6, 8 -> 7, 9 -> 8
for p in doc.paragraphs:
    txt = p.text.strip()
    if txt.startswith("7. Limitations Analysis"):
        p.text = "6. Limitations Analysis"
    elif txt.startswith("7.1 Methodological Limitations"):
        p.text = "6.1 Methodological Limitations"
    elif txt.startswith("7.1.1 Methodological Clarification"):
        p.text = "6.1.1 Methodological Clarification on Synthetic Data vs. Privacy-Preserving Computation"
    elif txt.startswith("8. Future Work"):
        p.text = "7. Future Work"
    elif txt.startswith("9. Conclusion") or txt.startswith("8. Conclusion"):
        p.text = "8. Conclusion"
...
# V7 Table of Contents entries: Clean 8 sections
toc_entries = [
    ("1. Introduction", "4", 1),
    ("2. Related Work", "5", 1),
    ("3. Methodology", "7", 1),
    ...
    ("6. Limitations Analysis", "21", 1),
    ("6.1 Methodological Limitations", "21", 2),
    ("7. Future Work", "22", 1),
    ("8. Conclusion", "22", 1),
...
doc.save(v7_docx_path)
print(f"[SUCCESS] Saved clean PaperV7_Ollama_Primary.docx with Section 6 removed & renumbered!")

# Export PDF via Word COM Automation
print(f"Exporting PDF via Word COM Automation: {v7_pdf_path.name}...")
abs_docx = os.path.abspath(str(v7_docx_path))
abs_pdf = os.path.abspath(str(v7_pdf_path))
...
try:
    ...
    doc_word.Close()
    print(f"[SUCCESS] Exported high-quality PDF: {v7_pdf_path.name} ({page_count} Pages!)")
