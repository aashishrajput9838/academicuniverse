...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **`clean`** | 90 | 66.67% | 1.0000 | 1.0000 | 100.00% | 0.00% | 0.00% |
| **`scanner_copy`** | 90 | 66.67% | 1.0000 | 1.0000 | 100.00% | 0.00% | 0.00% |
| **`mobile_camera`**| 90 | 66.67% | 1.0000 | 1.0000 | 100.00% | 0.00% | 0.00% |
| **`rotated_90`** | 90 | 66.67% | 1.0000 | 1.0000 | 100.00% | 0.00% | 0.00% |
| **Overall Dataset** | **360** | **66.67%** | **1.0000** | **1.0000** | **100.00%** | **0.00%** | **0.00%** |

#### Empirical Analysis of Discovered Failure Modes:
1. **Key-Value Extraction Robustness (100% F1, 0.00% CER)**: Under zero-shot structured text extraction, Llama 3.1 8B achieved perfect field extraction precision (1.0000) and recall (1.0000) across all 360 specimens, demonstrating immunity to synthetic scanner noise, mobile camera perspective degradation, and 90° rotation.
2. **Category Misclassification Discovery (66.67% Category Accuracy)**: The empirical evaluation uncovered a prompt-level constraint failure mode. Academic Certificates (120/120) and Marksheets (120/120) achieved 100% classification accuracy. However, because `STUDENT_ID` was omitted from the prompt's `ALLOWED_CATEGORIES` list, the model strictly mapped Student ID cards (120/120) to `CERTIFICATE` (119/120) and `MARKSHEET` (1/120), highlighting the sensitivity of zero-shot document classification to schema completeness.
