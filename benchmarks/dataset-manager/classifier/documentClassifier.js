...
class DocumentClassifier {
    ...
    classify(filename, rawTextSnippet) {
        const lowerName = filename.toLowerCase();
        const text = (rawTextSnippet || '').toLowerCase();
        // 1. Marksheet
        if (/mark[s]?|mark\s*sheet|sem[ester\s]*\d|intermediate|cgpa|sgpa|result|\bms_\d|\bsynth_ms_/i.test(lowerName) ||
        ...
        if (/certific|completion|award|ethics|oracle|owasp|udemy|coursera|nptel|java_fundamentals|\bcert_\d|\bsynth_cert_/i.test(lowerName) ||
    ...
    getDestinationSubfolder(category) {
        const map = {
