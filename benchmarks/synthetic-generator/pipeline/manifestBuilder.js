...
class ManifestBuilder {
    ...
    static saveManifestAndMetadata(outputDir, seed, generatorVersion, entries) {
        ...
        const manifestHash = 'SHA256_' + crypto_1.default.createHash('sha256').update(deterministicPayload).digest('hex').substring(0, 16);
    ...
    static saveReport(outputDir, report) {
        const lines = [
            ...
            `| :--- | :---: | :---: |`,
            ...Object.entries(report.categoryBreakdown).map(([cat, count]) => `| ${cat} | ${count} | ${((count / report.totalDocuments) * 100).toFixed(1)}% |`),
            ``,
            `## Quality Profile Distribution`,
            `| Quality Profile | Document Count | Percentage |`,
            `| :--- | :---: | :---: |`,
            ...Object.entries(report.qualityProfileBreakdown).map(([q, count]) => `| ${q} | ${count} | ${((count / report.totalDocuments) * 100).toFixed(1)}% |`),
            ``,
            `## University Template Distribution`,
            `| Template ID | Document Count | Percentage |`,
            `| :--- | :---: | :---: |`,
            ...Object.entries(report.templateBreakdown).map(([t, count]) => `| ${t} | ${count} | ${((count / report.totalDocuments) * 100).toFixed(1)}% |`),
            ``,
            `---`,
            `*Report generated automatically by Academic Universe Research Platform.*`,
        ];
        const reportPath = path_1.default.join(outputDir, 'generation-report.md');
