...
async function seed() {
    try {
        ...
        const allPermissions = await models_1.Permission.find().lean();
        // 2. Create Organization
        let organization = await models_1.Organization.findOne({ slug: 'sharda-university' });
        if (!organization) {
            organization = await models_1.Organization.create({
        ...
        for (const roleConfig of roleConfigs) {
            ...
            if (!role) {
                role = (await models_1.Role.create({
                    name: roleConfig.name,
                    organizationId: organization._id,
            ...
            await models_1.RolePermission.insertMany(rolePermissions, {
                ordered: false,
            }).catch((error) => {
                if (error.code === 11000) {
        ...
        // 4. Create Demo Users
        const superAdminRole = await models_1.Role.findOne({
            name: 'SUPER_ADMIN',
            organizationId: organization._id,
        });
        ...
        const facultyRole = await models_1.Role.findOne({
            name: 'FACULTY',
            organizationId: organization._id,
        });
        const studentRole = await models_1.Role.findOne({
            name: 'STUDENT',
            organizationId: organization._id,
        });
        const demoUsers = [
        ...
        for (const userData of demoUsers) {
            let user = await models_1.User.findOne({ email: userData.email });
            if (!user) {
                user = await models_1.User.create(userData);
        ...
        // Set super admin
        const superAdmin = await models_1.User.findOne({ email: 'superadmin@academicuniverse.com' });
