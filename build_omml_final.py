...
def make_inline_omml(text):
    """
    Wraps an inline mathematical symbol or expression into native Word OMML (<m:oMath>).
    This ensures Word renders the symbol using native Cambria Math typography.
    """
...
def make_display_omml(eq_id, raw_text=""):
    """
    Constructs a 100% compliant Microsoft Word OMML Display Equation (<m:oMathPara>)
    with native Cambria Math typography and right-aligned IEEE equation tags: (1), (2), (3), (4), (5).
    """
    if eq_id == 1: # Velocity
        xml = f'''
        ...
        '''
    elif eq_id == 10: # Section 5.1 SIE-1.0 formula
        xml = f'''
        <m:oMathPara {nsdecls("m")}>
          <m:oMath>
        ...
        xml = f'''
        <m:oMathPara {nsdecls("m")}>
          <m:oMath><m:r><m:t>{raw_text}</m:t></m:r></m:oMath>
        </m:oMathPara>
        '''
    return parse_xml(xml)
...
def parse_line_to_paragraph(paragraph, text, default_font_size=11):
    """
    Parses Markdown tokens (**bold**, *italic*, `code`, $math$) and appends native Word runs or OMML math elements.
    """
    pattern = re.compile(r'(\*\*.*?\*\*|\*.*?\*|`.*?`|\$.*?\$|\$\\mu_v\$|\$\\lambda\$|\$\\mathcal\{H\}\$|\$\\epsilon\$|\$\\Delta\$)')
    ...
    for token in tokens:
        ...
        if token.startswith('**') and token.endswith('**'):
            ...
            # Insert native inline OMML math element
            omml = make_inline_omml(token)
            paragraph._p.append(omml)
        else:
            clean_s = clean_text_str(token)
            r = paragraph.add_run(clean_s)
            r.font.name = 'Times New Roman'
            r.font.size = Pt(default_font_size)
...
def build_omml_final_submission():
    ...
    while i < len(lines):
        ...
        # Centered Display Equations with Native OMML Engine
            ...
            eq_num = num_match.group(1) or num_match.group(2) if num_match else str(display_eq_count)

            # Build OMML Display Equation
            omml_elem = make_display_omml(display_eq_count, raw_eq)
