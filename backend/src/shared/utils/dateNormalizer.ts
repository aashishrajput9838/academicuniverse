 ...
 *
 * This is the ONLY place where date strings are parsed and normalized.
 * All modules must use this utility instead of direct `new Date()` or `Date.parse()`.
 *
 * Canonical output format: YYYY-MM-DD (ISO date only)
 */
...
export interface NormalizedDate {
  ...
  isoDateTime: string | null; // YYYY-MM-DDTHH:mm:ss.sssZ or null
  isValid: boolean;
  raw: string | number | Date | null | undefined;
}

/**
 * Attempts to parse a date value from any supported format.
 * Returns a normalized date object with canonical ISO formats.
 */
export function normalizeDate(value: DateInput): NormalizedDate {
  ...
  if (typeof value === 'number') {
    // Excel serial number or Unix timestamp
    let date: Date;
    if (value > 1000000000 && value < 9999999999) {
      // Likely Excel serial (days since 1899-12-30) or Unix timestamp in seconds
      if (value < 50000) {
        // Excel serial number (days since 1899-12-30, Lotus 1-2-3 bug)
    ...
    if (isNaN(date.getTime())) {
      console.warn('[DateNormalizer] Invalid numeric date received', { raw });
      return { iso: null, isoDateTime: null, isValid: false, raw };
    }
    return {
  ...
  if (typeof value !== 'string') {
    console.warn('[DateNormalizer] Unsupported date type', { raw, type: typeof value });
    return { iso: null, isoDateTime: null, isValid: false, raw };
  }
  ...
  // Try RFC format (e.g., "Mon, 15 Jan 2024 10:30:00 +0000")
  ...
  // Try month-name formats (e.g., "January 15, 2024", "Jan 15, 2024")
  ...
  // Try day-name formats (e.g., "Monday, January 15, 2024")
  ...
  // Try DD/MM/YYYY or MM/DD/YYYY (ambiguous, prefer DD/MM/YYYY)
  ...
  console.warn('[DateNormalizer] Unable to normalize date', { raw });
  return { iso: null, isoDateTime: null, isValid: false, raw };
}

/**
 * Normalize an array of schedule dates.
 * Returns a new array with normalized date strings.
 */
