...
exports.TimetableGenerator = void 0;
...
class TimetableGenerator {
    static async generate(data, template) {
        ...
        let currentX = 40;
        headers.forEach((h, i) => {
            page.drawText(h, { x: currentX, y: tableY, size: 8.5, font: fontBold, color: (0, pdf_lib_1.rgb)(1, 1, 1) });
        ...
        daysOrDates.forEach((label, rIdx) => {
            ...
            let rX = 40;
            page.drawText(label, { x: rX, y: rowY, size: 8, font: fontBold, color: (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2) });
            rX += colWidths[0];
            if (isExam) {
                const course = subjects[rIdx % subjects.length] || { courseCode: 'CS101', courseName: 'Data Structures' };
                page.drawText('10:00 AM - 01:00 PM', { x: rX, y: rowY, size: 8, font, color: (0, pdf_lib_1.rgb)(0.3, 0.3, 0.3) });
                rX += colWidths[1];
                page.drawText(course.courseCode, { x: rX, y: rowY, size: 8, font: fontBold, color: primaryRgb });
                rX += colWidths[2];
                page.drawText(course.courseName, { x: rX, y: rowY, size: 8, font, color: (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2) });
                rX += colWidths[3];
                page.drawText(`Hall ${101 + rIdx}`, { x: rX, y: rowY, size: 8, font, color: (0, pdf_lib_1.rgb)(0.3, 0.3, 0.3) });
            }
            else {
                ['Lect: CS101', 'Lab: CS102', 'Lect: CS103', 'Project Lab'].forEach((slot) => {
                    page.drawText(slot, { x: rX, y: rowY, size: 8, font, color: (0, pdf_lib_1.rgb)(0.3, 0.3, 0.3) });
                    rX += colWidths[1];
                });
            }
            rowY -= 22;
        ...
        page.drawText('1. Students must carry valid Student ID & Admit Card to all examination halls.', { x: 45, y: 110, size: 7.5, font, color: (0, pdf_lib_1.rgb)(0.4, 0.4, 0.4) });
        page.drawText('2. Electronic gadgets & unauthorized materials are strictly prohibited.', { x: 45, y: 98, size: 7.5, font, color: (0, pdf_lib_1.rgb)(0.4, 0.4, 0.4) });
