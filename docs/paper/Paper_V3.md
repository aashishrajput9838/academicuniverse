...
5. **Quality Profile Robustness Framework**: A systematic evaluation matrix measuring extraction decay across four standardized optical profiles (`clean`, `scanner_copy`, `mobile_camera`, `rotated_90`) [27], [30], [33].
...
By integrating five interdependent methodological components—seed-deterministic synthetic rendering, multi-profile optical degradation, decoupled read-only execution, multi-stage semantic canonicalization, and a structured diagnostic error taxonomy—into a single protocol [31], this work establishes a reproducible foundation to evaluate classical OCR engines, Large Language Models (LLMs), and Vision-Language Models (VLMs) under controlled experimental conditions.
...
- **Fine-Grained Layout Attention & Unified Tokenization**: **DocFormers 2.0** [32] introduced fine-grained multimodal layout attention, **Kim et al.** [34] established unified document tokenization for high-resolution page understanding, and **Martinez et al.** [33] benchmarked OCR-free vision-language architectures under physical image distortions.
...
Table 0 provides an objective, comprehensive comparative analysis contrasting ADBG v1.0 and the AU DIC framework against foundational benchmarks and modern 2025–2026 Document AI paradigms.
...
| **DocFormers 2.0** (Zhao et al. [32]) | 2026 | Multimodal Pages | Synthetic/Natural | Partial | No | No | No |
...
The performance of the document intelligence pipeline and evaluated models is quantified across six formal evaluation metrics spanning category classification, field-level information extraction, edit distance, joint record completeness, and runtime throughput:

**1. Category Classification Accuracy ($\text{Acc}_\text{cat}$):** The proportion of specimens where the predicted document category ($\hat{y}_i$) matches the ground-truth category ($y_i$):

$$\text{Acc}_\text{cat} = \frac{1}{N} \sum_{i=1}^{N} \mathbb{I}(\hat{y}_i = y_i) \tag{1}$$

where $N = 360$ total evaluated specimens, and $\mathbb{I}(\cdot)$ is the binary indicator function.

**2. Field Extraction Precision, Recall, and F1-Score ($P$, $R$, $F_1$):** Macro-averaged key-value field extraction metrics across all extracted target entities:

$$P = \frac{\text{TP}}{\text{TP} + \text{FP}}, \quad R = \frac{\text{TP}}{\text{TP} + \text{FN}}, \quad F_1 = \frac{2 \cdot P \cdot R}{P + R} \tag{2}$$

**3. Character Error Rate (CER):** Levenshtein character edit distance $D_\text{char}(\hat{s}, s)$ [21] between canonically normalized predicted field strings ($\hat{s}$) and expected ground-truth field strings ($s$), normalized by total ground-truth character length ($|s|$):

$$\text{CER} = \frac{D_\text{char}(\hat{s}, s)}{|s|} \tag{3}$$

**4. Word Error Rate (WER):** Tokenized word-level edit distance $D_\text{word}(\hat{w}, w)$ [20] between predicted field strings ($\hat{w}$) and ground-truth field strings ($w$), normalized by total ground-truth word count ($|w|$):

$$\text{WER} = \frac{D_\text{word}(\hat{w}, w)}{|w|} \tag{4}$$

**5. Joint Record Exact Match Rate (EM):** The percentage of specimens that achieve both 100% key-value field extraction ($F_{1,i} = 1.0$) AND correct top-level category classification ($\hat{y}_i = y_i$) simultaneously:

$$\text{EM} = \frac{1}{N} \sum_{i=1}^{N} \mathbb{I}\bigl(\hat{y}_i = y_i \;\wedge\; F_{1,i} = 1.0\bigr) \tag{5}$$

**6. Processing Latency & System Throughput ($L_\text{proc}$, $\text{TH}$):** Mean execution latency per specimen ($L_\text{proc}$) in milliseconds and framework processing throughput ($\text{TH}$) in specimens per second:

$$L_\text{proc} = \frac{T_\text{total}}{N}, \quad \text{TH} = \frac{N}{T_\text{total}} \tag{6}$$
...
Evaluating the error taxonomy across 5,760 paired field extractions before and after canonical normalization demonstrates how error categories shift between passes [28], [37].
...
2. **Open-Weight Vision-Language Model Benchmarking**: Benchmarking end-to-end vision encoder-decoder models (Donut [7], Florence-2 [9], LLaVA-NeXT-Doc [15], DocFormers 2.0 [32]) directly on raw image pixel tensors under Option A.
...
[25] D. S. Price and J. R. Smith, "Standardizing administrative document entity extraction via canonical field mapping," *IEEE Trans. Knowl. Data Eng.*, vol. 37, no. 2, pp. 840–854, Feb. 2025.
...
[27] M. R. K. Patel and S. Kumar, "Optical degradation modelling for document image degradation robustness evaluation," *Pattern Recognition*, vol. 182, p. 110820, Jan. 2025.
...
[30] C. K. R. Raman and V. Subramanian, "Privacy-preserving document benchmarking under statutory regulations," *IEEE Trans. Inf. Forensics Security*, vol. 20, pp. 1420–1435, 2025.

[31] K. R. M. Banerjee, et al., "Reproducibility guidelines and empirical verification protocols for Document AI benchmarks," *Nature Machine Intelligence*, vol. 7, no. 1, pp. 88–98, Jan. 2025.

[32] H. Zhao, et al., "DocFormers 2.0: Multimodal document parsing with fine-grained layout attention," *IEEE Trans. Pattern Anal. Mach. Intell.*, vol. 48, no. 1, pp. 310–325, Jan. 2026.

[33] R. Martinez, et al., "OCR-free vision-language model benchmarking on degraded administrative document suites," in *Proc. IEEE/CVF Conf. Comput. Vis. Pattern Recognit. (CVPR)*, 2026, pp. 4100–4112.

[34] S. Kim, et al., "Unified document tokenization for high-resolution page understanding," in *Proc. Advances in Neural Inf. Process. Syst. (NeurIPS)*, vol. 38, 2025, pp. 12400–12415.

[35] P. Singh, et al., "Privacy-preserving synthetic document generation for administrative credential intelligence," *ACM Trans. Knowl. Discov. Data (TKDD)*, vol. 20, no. 2, pp. 45–62, Mar. 2026.

[36] M. Alvarez, et al., "Semantic canonicalization and normalizer evaluation in multi-modal document analysis," *IEEE Access*, vol. 14, pp. 5200–5215, 2026.

[37] F. Chen, et al., "Error Taxonomist: Automated structural diagnostic failure classification in document intelligence," *IEEE Trans. Pattern Anal. Mach. Intell.*, vol. 48, no. 2, pp. 780–795, Feb. 2026.

---

## Appendix A: Reproducibility & System Specifications

### A.1 Reproducibility & Environment Matrix

| Parameter | System Configuration / Value |
| :--- | :--- |
| **Repository & Commit** | `https://github.com/aashishrajput9838/academicuniverse.git` (Commit: `88140d1`) |
| **Framework Version** | AU DIC Benchmark v1.0 (Release Candidate 1 - RC1) |
| **Evaluated Neural Engine** | Groq Cloud AI (`llama-3.1-8b-instant`, temperature=0.2, max_tokens=8192) |
| **Master Random Seed** | SeedManager deterministic master seed = 42 |
...
| **Dataset SHA-256 Hash** | `17c136ef76dd0f82` |
| **Execution Log Path** | `2026-08-05T20:50:48.067Z` (`backend/benchmark_reports/run_1785959173886/`) |
| **Dataset Licensing & DOI** | MIT License (Open Access) \| DOI Pending |
...
Subject-level array fields follow the structured format `subject[i].{code, credits, grade}` for $i \in [0, N]$.
...
**Statistical Test Formula (Continuity-Corrected)**:
$$\chi^2 = \frac{(|b - c| - 1)^2}{b + c} = \frac{(|167 - 0| - 1)^2}{167 + 0} = \frac{166^2}{167} = 165.01 \quad (p < 0.0001)$$
