...
exports.AdmitCardGenerator = void 0;
...
class AdmitCardGenerator {
    static async generate(data, template) {
        ...
        fields.forEach((f, idx) => {
            page.drawText(f.label, { x: 45, y: infoY - idx * 17, size: 9, font: fontBold, color: (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2) });
            page.drawText(f.val, { x: 155, y: infoY - idx * 17, size: 9, font, color: (0, pdf_lib_1.rgb)(0.1, 0.1, 0.1) });
        });
        ...
        page.drawText('AFFIX RECENT', { x: photoX + 15, y: photoY + 65, size: 7.5, font, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        page.drawText('PASSPORT PHOTO', { x: photoX + 12, y: photoY + 52, size: 7.5, font, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        ...
        page.drawText('Course Code', { x: 50, y: tableY - 20, size: 8.5, font: fontBold, color: (0, pdf_lib_1.rgb)(1, 1, 1) });
        page.drawText('Course Title', { x: 140, y: tableY - 20, size: 8.5, font: fontBold, color: (0, pdf_lib_1.rgb)(1, 1, 1) });
        page.drawText('Invigilator Sign', { x: 420, y: tableY - 20, size: 8.5, font: fontBold, color: (0, pdf_lib_1.rgb)(1, 1, 1) });
        let rY = tableY - 42;
        courses.forEach((c, idx) => {
            page.drawText(c.courseCode, { x: 50, y: rY, size: 8.5, font: fontBold, color: (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2) });
            page.drawText(c.courseName, { x: 140, y: rY, size: 8.5, font, color: (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2) });
            page.drawText('[  SIGN HERE  ]', { x: 420, y: rY, size: 7.5, font, color: (0, pdf_lib_1.rgb)(0.6, 0.6, 0.6) });
            rY -= 18;
        });
        // Student & Controller Signature Line
        page.drawLine({ start: { x: 45, y: 110 }, end: { x: 180, y: 110 }, thickness: 1, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
        page.drawText("Candidate's Signature", { x: 55, y: 95, size: 8.5, font, color: (0, pdf_lib_1.rgb)(0.3, 0.3, 0.3) });
        page.drawLine({ start: { x: width - 200, y: 110 }, end: { x: width - 65, y: 110 }, thickness: 1, color: (0, pdf_lib_1.rgb)(0.5, 0.5, 0.5) });
...
exports.AdmitCardGenerator = AdmitCardGenerator;
