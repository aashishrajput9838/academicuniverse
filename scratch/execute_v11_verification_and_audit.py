...
manifest_v11_path = workspace / "docs" / "paper" / "PAPER_V11_RELEASE_MANIFEST.md"
audit_v11_path = workspace / "docs" / "paper" / "PAPER_V11_CHANGE_AUDIT.md"

print("=================================================================")
print(" EXECUTING VERIFICATION & AUDIT FOR PAPER V11 REVISION (NO APPENDIX TABLES)")
print("=================================================================")

# 1. Verify V5, V6, V7, V8, V9 & V10 Artifacts Frozen Status
...
# 2. Verify V11 Artifacts Existence
assert v11_docx.exists(), "CRITICAL: PaperV11_Ollama_Primary.docx missing!"
assert v11_pdf.exists(), "CRITICAL: PaperV11_Ollama_Primary.pdf missing!"
assert v11_md.exists(), "CRITICAL: Paper_V11.md missing!"
print("[PASS] Paper V11 artifacts exist.")

# 3. Inspect V11 DOCX Sections 1-7 Structure
doc = docx.Document(v11_docx)
...
assert sec1_idx is not None, "Error: Section 1 heading not found in V11 DOCX body!"
assert sec2_idx is not None, "Error: Section 2 heading not found in V11 DOCX body!"
assert sec3_idx is not None, "Error: Section 3 heading ('3. Methodology') not found in V11 DOCX body!"
assert sec4_idx is not None, "Error: Section 4 heading not found in V11 DOCX body!"
assert sec5_idx is not None, "Error: Section 5 heading ('5. Results & Discussion') not found in V11 DOCX body!"
assert sec6_idx is not None, "Error: Section 6 heading ('6. Future Work') not found in V11 DOCX body!"
assert sec7_idx is not None, "Error: Section 7 heading ('7. Conclusion') not found in V11 DOCX body!"
assert ack_idx is not None, "Error: ACKNOWLEDGMENT heading not found in V11 DOCX body!"
assert ref_idx is not None, "Error: REFERENCES heading not found in V11 DOCX body!"
...
print(f"[INFO] Total tables in V11 DOCX: {total_tables}")
assert total_tables == 13, f"Error: Expected exactly 13 tables in V11 DOCX, found {total_tables}!"
...
print("[PASS] Table of Contents in V11 DOCX verified synchronized, clean, and renumbered (7 sections + ACK + REFERENCES).")

# Check PDF Page Count and Visual Integrity via pypdf
reader = pypdf.PdfReader(str(v11_pdf))
pdf_page_count = len(reader.pages)
print(f"[INFO] V11 PDF Page Count: {pdf_page_count} pages.")
assert pdf_page_count >= 20, f"Error: Unexpected page count {pdf_page_count}"

# Generate Release Manifest & Change Audit
sha_docx = hashlib.sha256(v11_docx.read_bytes()).hexdigest()
sha_pdf = hashlib.sha256(v11_pdf.read_bytes()).hexdigest()
sha_md = hashlib.sha256(v11_md.read_bytes()).hexdigest()

manifest_content = f"""# PAPER V11 RELEASE MANIFEST & SCIENTIFIC ARTIFACT AUDIT
...
| :--- | :---: | :--- | :---: |
| `docs/paper/PaperV11_Ollama_Primary.docx` | {v11_docx.stat().st_size:,} | `{sha_docx}` | **VERIFIED** |
| `docs/paper/PaperV11_Ollama_Primary.pdf` | {v11_pdf.stat().st_size:,} | `{sha_pdf}` | **VERIFIED ({pdf_page_count} Pages)** |
| `docs/paper/Paper_V11.md` | {v11_md.stat().st_size:,} | `{sha_md}` | **VERIFIED** |
...
manifest_v11_path.write_text(manifest_content, encoding="utf-8")
print(f"[SUCCESS] Written {manifest_v11_path.name}")

audit_content = f"""# PAPER V11 CHANGE AUDIT REPORT: APPENDIX TABLES REMOVAL

**Date:** August 22, 2026  
**Auditor:** Automated Publication Pipeline & Verification Subsystem  
**Scope:** Paper V11 revision (`PaperV11_Ollama_Primary.docx`, `PaperV11_Ollama_Primary.pdf`, `Paper_V11.md`)

---

## 1. Executive Summary of Appendix Tables Removal

As directed, all tables lingering after Conclusion (from the previous appendices: Reproducibility Table, Observation Derivation Table, Confusion Matrix Table, McNemar Table, Bootstrap CI Table) have been completely removed from the document XML.
...
## 2. Structural Verification Matrix: V10 vs. V11

| Section / Element | Paper V10 | Paper V11 | Status |
| :--- | :--- | :--- | :---: |
| **Appendix Tables After Conclusion** | 5 Tables Present | **COMPLETELY REMOVED (0 Tables After Conclusion)** | **VERIFIED** |
| **Total Tables in Document** | 18 Tables | **EXACTLY 13 Tables** | **VERIFIED** |
| **TOC Synchronization** | 7 sections + ACK + REF | **Clean 7 sections + ACK + REF** | **VERIFIED** |
| **Page Layout** | IEEE Access Double Column (24 Pages) | **IEEE Access Double Column ({pdf_page_count} Pages)** | **VERIFIED** |
...
audit_v11_path.write_text(audit_content, encoding="utf-8")
print(f"[SUCCESS] Written {audit_v11_path.name}")
print("=================================================================")
print(" ALL V11 VERIFICATION AND AUDIT CHECKS PASSED!")
print("=================================================================")
