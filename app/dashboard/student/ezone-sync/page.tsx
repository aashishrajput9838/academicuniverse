...
import { Loader2, RefreshCw, CheckCircle2, AlertCircle, User, BookOpen, GraduationCap, Calendar } from 'lucide-react';
    ...
    const fetchProfile = async () => {
        try {
            ...
            if (res.success && res.data) {
                setProfile(res.data);
                setStep(4); // Show profile if already synced
            }
        } catch (error) {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 p-6">
            <div className="space-y-2">
                <h1 className="text-3xl font-bold text-white">Ezone Sync</h1>
                    ...
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input 
                                ...
                                className="bg-slate-900/50 border-slate-700 text-white text-center text-2xl tracking-widest"
                                maxLength={6}
                            />
                        </div>
                        <Button 
                        ...
                        <Button variant="link" onClick={() => setStep(1)} className="w-full text-slate-400 hover:text-white text-sm">
            ...
            {step === 4 && profile && (
                <div className="space-y-6 animate-in fade-in duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <StatCard icon={<BookOpen className="text-orange-400" />} label="Subjects" value={profile.subjects.length} />
                    </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {profile.subjects.map((sub: any, i: number) => (
                                        <div key={i} className="flex items-center justify-between p-3 bg-slate-900/30 rounded-lg border border-slate-700/30">
