...
import {
  ...
  getRoutingInfo,
  softDeleteDocument,
  bulkDeleteReviewRequiredDocuments,
...
export function GrowthUploadPanel({ backendToken }: GrowthUploadPanelProps) {
  ...
  const handleConfirmBulkDelete = async () => {
    if (isBulkDeleting || pendingReview.length === 0) return;

    setIsBulkDeleting(true);
    setBulkDeleteToast(null);

    try {
      const result = await bulkDeleteReviewRequiredDocuments(backendToken);

      if (result.deletedProcessingIds && result.deletedProcessingIds.length > 0) {
        bulkRemoveUploads(result.deletedProcessingIds);
      }
      await loadHistory(backendToken);
      ...
      if (result.failedCount > 0) {
        setBulkDeleteToast({
          type: 'warning',
          message: `Deleted ${result.successfullyDeleted} Review Required files, but ${result.failedCount} file artifact(s) could not be cleaned up completely.`,
        });
      } else {
        setBulkDeleteToast({
          type: 'success',
          message: `Successfully deleted ${result.successfullyDeleted} Review Required file${result.successfullyDeleted !== 1 ? 's' : ''}.`,
        });
      }
    } catch (err: any) {
      setShowBulkDeleteModal(false);
      setBulkDeleteToast({
        type: 'error',
        message: err?.message || 'Failed to delete Review Required files. Please try again.',
      });
    } finally {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-violet-500/30 bg-violet-500/5 backdrop-blur-sm p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="rounded-full bg-violet-500/20 border border-violet-500/30 px-2.5 py-0.5 text-sm font-bold text-violet-300">
            ...
            <button
              type="button"
              id="bulk-delete-review-required-btn"
              disabled={pendingReview.length === 0 || isBulkDeleting}
              title={pendingReview.length === 0 ? 'No Review Required files to delete.' : 'Delete All Review Required Files'}
              onClick={() => setShowBulkDeleteModal(true)}
              className={`flex items-center gap-2 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all shadow-sm ${
                pendingReview.length === 0
                  ? 'bg-red-500/5 text-red-400/40 border border-red-500/15 cursor-not-allowed opacity-60'
                  : 'bg-red-500/15 text-red-300 border border-red-500/30 hover:bg-red-500/25 hover:border-red-500/50 hover:text-red-200 cursor-pointer active:scale-95'
              }`}
            >
              {isBulkDeleting ? (
                <>
                  <svg className="h-3.5 w-3.5 animate-spin text-red-300" viewBox="0 0 24 24" fill="none">
                ...
                <>
                  ...
                  <span>Delete All Review Required</span>
                </>
              )}
            </button>
          </div>
      ...
      {showBulkDeleteModal &&
        ...
        createPortal(
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="w-full max-w-md rounded-2xl border border-red-500/30 bg-slate-900 p-6 shadow-2xl shadow-red-950/40">
              <div className="flex items-center gap-3 text-red-400 mb-4">
                ...
                <h3 className="text-lg font-bold text-white">Delete All Review Required Files?</h3>
              </div>

              <div className="space-y-3 text-sm text-slate-300">
                <p>You are about to permanently delete every document currently marked as <strong>&ldquo;Review Required&rdquo;</strong>.</p>
                
                <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-3 flex items-center justify-between text-xs">
                  <span className="text-red-300 font-medium">Files to delete:</span>
                  <span className="rounded-full bg-red-500/30 px-2.5 py-0.5 font-bold text-red-200">{pendingReview.length}</span>
                </div>

                <p className="text-xs text-slate-400 italic">This action cannot be undone.</p>
              </div>
              ...
              <div className="mt-6 flex items-center justify-end gap-3">
                <button
                  ...
                  className="rounded-xl border border-slate-700 bg-slate-800 px-4 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isBulkDeleting || pendingReview.length === 0}
                  onClick={handleConfirmBulkDelete}
                  className="flex items-center gap-2 rounded-xl bg-red-600 hover:bg-red-500 text-white px-4 py-2 text-xs font-bold transition-colors shadow-lg shadow-red-950/50 disabled:opacity-50 cursor-pointer"
                >
                  {isBulkDeleting ? (
                    <>
                    ...
                    <span>Delete {pendingReview.length} Files</span>
                  )}
                </button>
              </div>
            </div>
      ...
      {bulkDeleteToast && (
        <div
          className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-2xl border px-4 py-3 shadow-2xl backdrop-blur-md animate-in slide-in-from-bottom-4 duration-300 ${
            bulkDeleteToast.type === 'success'
              ? 'border-emerald-500/40 bg-emerald-950/90 text-emerald-200'
              : bulkDeleteToast.type === 'warning'
              ? 'border-amber-500/40 bg-amber-950/90 text-amber-200'
              : 'border-red-500/40 bg-red-950/90 text-red-200'
          }`}
        >
          <span className="text-xs font-medium">{bulkDeleteToast.message}</span>
          <button
            type="button"
            onClick={() => setBulkDeleteToast(null)}
            className="ml-2 rounded-lg p-1 hover:bg-white/10 text-current opacity-70 hover:opacity-100 cursor-pointer"
          >
            ✕
          </button>
        </div>
