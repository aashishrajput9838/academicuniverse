...
async function runPhaseB() {
  console.log('============================================================');
  console.log(' AU DIC BENCHMARK — PHASE B: 10-IMAGE SANITY RUN (LOCAL OLLAMA)');
  console.log('============================================================');
  ...
  const allGtFiles = require('fs').readdirSync(gtFolder)
    .filter((f: string) => f.endsWith('.json'))
    .map((f: string) => path.join('groundtruth', f));

  // Pick 10 representative samples across document categories & quality profiles
  const selectedRelPaths = allGtFiles.slice(0, 10);
  console.log(`Selected ${selectedRelPaths.length} samples for Phase B sanity evaluation.`);
  ...
  const runId = `run_phase_b_${Date.now()}`;
  ...
  console.log('\n============================================================');
  console.log(' PHASE B 10-SAMPLE SANITY RUN SUMMARY');
  console.log('============================================================');
  ...
  console.log('\n✅ PHASE B SANITY RUN COMPLETE.');
}

runPhaseB().catch((err) => {
  console.error('\n❌ PHASE B SANITY RUN FAILED:', err);
  process.exit(1);
});
