...
export class MarksheetGenerator {
  static async generate(data: SyntheticDocumentData, template: ITemplatePlugin): Promise<Uint8Array> {
    ...
    studentInfo.forEach((item, idx) => {
      ...
      page.drawText(item.label, { x, y, size: 9, font: fontBold, color: rgb(0.2, 0.2, 0.2) });
      page.drawText(item.value, { x: x + 110, y, size: 9, font, color: rgb(0.1, 0.1, 0.1) });
    });
    ...
    let currentX = 45;
    headers.forEach((h, i) => {
      page.drawText(h, { x: currentX, y: tableY, size: 9, font: fontBold, color: rgb(1, 1, 1) });
    ...
    if (currentSem) {
      currentSem.courseMarks.forEach((course, rIdx) => {
        ...
        let rX = 45;
        page.drawText(course.courseCode, { x: rX, y: rowY, size: 8.5, font: fontBold, color: rgb(0.2, 0.2, 0.2) });
        rX += colWidths[0];
        page.drawText(course.courseName, { x: rX, y: rowY, size: 8.5, font, color: rgb(0.2, 0.2, 0.2) });
        rX += colWidths[1];
        page.drawText(String(course.credits), { x: rX + 15, y: rowY, size: 8.5, font, color: rgb(0.2, 0.2, 0.2) });
        rX += colWidths[2];
        page.drawText(course.grade, { x: rX + 10, y: rowY, size: 8.5, font: fontBold, color: primaryRgb });
        rX += colWidths[3];
        page.drawText(`${course.marksObtained}/100`, { x: rX + 5, y: rowY, size: 8.5, font, color: rgb(0.2, 0.2, 0.2) });
    ...
    page.drawText('QR VERIFY', { x: 50, y: footerY, size: 6, font, color: rgb(0.5, 0.5, 0.5) });
