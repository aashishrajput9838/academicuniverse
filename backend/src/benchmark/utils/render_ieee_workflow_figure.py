"""
IEEE Publication Workflow Figure Generator v9.0 (PERFECT UNCLIPPED CANVAS + BOX PADDING FIX)
===========================================================================================
FIXES APPLIED:
1. Shifted all Y-coordinates so all 20 nodes range from y=215 (START) down to y=12 (END).
2. Canvas limits set to ylim: [0, 225], figsize: (8.5, 22.5 inches). ZERO clipping at bottom or top!
3. All text fits comfortably inside process boxes and callouts with 4-sided padding.
...
dot_content = """digraph G {
    graph [rankdir=TB, nodesep=0.5, ranksep=0.6, fontname="Times-Bold", fontsize=16, compound=true];
    node [shape=box, style="rounded,filled", fillcolor="#F8F9FA", color="#003366", penwidth=2.0, fontname="Times-Bold", fontsize=14];
    edge [color="#003366", penwidth=2.0, arrowsize=1.0];
...
# 2. Render 600 DPI PNG, SVG, and Vector PDF with 100% positive Y-coordinates (y=12 to 215) and ylim: [0, 225]
fig, ax = plt.subplots(figsize=(8.5, 22.5), dpi=600)
ax.set_xlim(0, 100)
ax.set_ylim(0, 225)
ax.axis("off")

# Title Banner
title_box = patches.FancyBboxPatch((3, 214), 94, 8.5, boxstyle="round,pad=0.4", fc="#003366", ec="#003366", lw=2.0)
ax.add_patch(title_box)
ax.text(50, 218.2, "AU DIC Framework — End-to-End Methodological Workflow", color="white", fontsize=17, fontweight="bold", ha="center", va="center", fontfamily="serif")

# Nodes data: All Y-coordinates are strictly positive (y=206 down to y=12)
# Generous box dimensions (w=68-78, h=6.5-7.5) ensure text fits comfortably inside boxes with padding!
nodes = [
    ("start", "START", "oval", 50, 206, 22, 5.5, "#003366", "#003366", "white"),
    ("config", "Research Configuration", "process", 50, 195, 60, 6.2, "#F8F9FA", "#003366", "black"),
    ("seed", "Deterministic Seed Initialization\n(PrngSeedGenerator)", "process", 50, 183, 68, 7.2, "#F8F9FA", "#003366", "black"),
    ("adbg", "ADBG Synthetic Academic Document Generation", "process", 50, 170, 74, 7.2, "#F8F9FA", "#003366", "black"),
    ("typst", "Typst Vector PDF Compilation", "process", 28, 156, 38, 6.2, "#F8F9FA", "#003366", "black"),
    ("gt_json", "Ground Truth JSON Assembly", "process", 72, 156, 38, 6.2, "#F8F9FA", "#003366", "black"),
    ("raster", "Raster Image Conversion (300 DPI)", "process", 28, 143, 38, 6.2, "#F8F9FA", "#003366", "black"),
    ("degradation", "Controlled Degradation Matrix\n(clean | scanner | camera | rotated)", "diamond", 28, 128, 40, 8.5, "#EDF2F7", "#2B6CB0", "#1A202C"),
    ("dataset", "AU_DIC_Benchmark_v1.0 Dataset Assembly\n(N = 360 Specimen Suite)", "process", 50, 113, 76, 7.2, "#E6FFFA", "#234E52", "#1A202C"),
    ("runner", "Benchmark Runner Engine Initialization\n(isReadOnly: true, allowMockFallback: false)", "process", 50, 101, 76, 7.2, "#F8F9FA", "#003366", "black"),
    ("inference", "Live Vision-Language / OCR Model Inference\n(Groq Cloud Llama 3.1 8B Instant)", "process", 50, 89, 76, 7.2, "#F8F9FA", "#003366", "black"),
    ("prediction", "Raw Prediction Extraction & JSON Parsing", "process", 50, 77, 70, 6.2, "#F8F9FA", "#003366", "black"),
    ("normalizer", "Six-Stage Semantic Canonical Normalization\n(CanonicalNormalizer)", "process", 50, 64, 76, 7.2, "#FEFCBF", "#744210", "#744210"),
    ("comparator", "Field-Level Candidate Comparison", "process", 50, 52, 66, 6.2, "#F8F9FA", "#003366", "black"),
    ("taxonomy", "Nine-Class Structured OCR Error Taxonomy\n(ErrorTaxonomist)", "process", 50, 40, 72, 7.2, "#F8F9FA", "#003366", "black"),
    ("metrics", "Multi-Task Metric Computation", "process", 50, 28, 64, 6.2, "#EBF8FF", "#2B6CB0", "#2B6CB0"),
    ("stats", "Statistical Significance & Uncertainty Analysis", "process", 50, 16, 70, 6.2, "#F0FFF4", "#22543D", "#22543D"),
    ("artifacts", "Publication Artifact Generation", "process", 50, 4, 64, 6.2, "#FAF5FF", "#553C9A", "#553C9A"),
    ("end", "END", "oval", 50, -7, 22, 5.5, "#003366", "#003366", "white"),
]

# Expand Y-limits to [-15, 225] so END node at y=-7 and Title at y=218 are 100% inside canvas
ax.set_ylim(-15, 225)
...
for nid, lbl, ntype, x, y, w, h, bg, border, tc in nodes:
    ...
    if ntype == "oval":
        ...
        ax.add_patch(patch)
        ax.text(x, y, lbl, color=tc, fontsize=14, fontweight="bold", ha="center", va="center", fontfamily="serif")
    elif ntype == "process":
        patch = patches.FancyBboxPatch((x - w/2, y - h/2), w, h, boxstyle="round,pad=0.4", fc=bg, ec=border, lw=2.0)
        ax.add_patch(patch)
        ax.text(x, y, lbl, color=tc, fontsize=12.5, fontweight="bold", ha="center", va="center", fontfamily="serif")
    elif ntype == "diamond":
...
print("Figure 2 full flowchart rendered with ZERO clipping from START to END and text padded inside boxes!")
