...
v9_docx_path = workspace / "docs" / "paper" / "PaperV9_Ollama_Primary.docx"
v9_pdf_path = workspace / "docs" / "paper" / "PaperV9_Ollama_Primary.pdf"

print("============================================================")
print(" GENERATING PAPER V9 (ETHICS & APPENDIX A REMOVED & V9 COMPILED)")
print("============================================================")
...
# -------------------------------------------------------------
# 5.3 REMOVE APPENDIX A (REPRODUCIBILITY) & RENUMBER APPENDICES
# -------------------------------------------------------------
app_a_start = None
app_b_start = None
for i, p in enumerate(doc.paragraphs):
    txt = p.text.strip()
    if (txt.startswith("APPENDIX A: REPRODUCIBILITY") or txt.startswith("APPENDIX A: Reproducibility") or "REPRODUCIBILITY & SYSTEM" in txt) and app_a_start is None:
        app_a_start = i
    elif (txt.startswith("APPENDIX B: FIELD") or txt.startswith("APPENDIX B: Field") or txt.startswith("APPENDIX B")) and app_b_start is None and app_a_start is not None:
        app_b_start = i
        break

if app_a_start is not None and app_b_start is not None:
    app_a_paras = [doc.paragraphs[k] for k in range(app_a_start, app_b_start)]
    for p_del in app_a_paras:
        delete_paragraph(p_del)

# Renumber Appendix B -> A, Appendix C -> B
for p in doc.paragraphs:
    txt = p.text.strip()
    if txt.startswith("The remainder of this paper is organized as follows"):
        p.text = "The remainder of this paper is organized as follows. Section 2 surveys related work and presents a literature comparison. Section 3 details the proposed methodology, including the decoupled system architecture and complete end-to-end data flow. Section 4 specifies the experimental setup, computing environment, dataset composition, parameter matrix, evaluation protocol, and mathematical formulations of metrics. Section 5 presents empirical validation results, statistical hypothesis testing, ablation studies, and the classical machine learning classification benchmark. Section 6 outlines future research directions, Section 7 concludes the paper, and the Appendices provide field derivations and empirical statistical benchmarks."
    elif txt.startswith("APPENDIX B: FIELD SPECIFICATION") or txt.startswith("APPENDIX B: Field Specification"):
        p.text = "APPENDIX A: FIELD SPECIFICATION & OBSERVATION COUNT DERIVATION"
    elif txt.startswith("B.1 Document Category Field Structure"):
        p.text = "A.1 Document Category Field Structure"
    elif txt.startswith("B.2 Mathematical Derivation"):
        p.text = "A.2 Mathematical Derivation of 24,480 Paired Observations"
    elif txt.startswith("APPENDIX C: EMPIRICAL STATISTICAL") or txt.startswith("APPENDIX C: Empirical Statistical"):
        p.text = "APPENDIX B: EMPIRICAL STATISTICAL METHODOLOGY & BENCHMARKS"
    elif txt.startswith("C.1 Empirical Category Confusion Matrix"):
        p.text = "B.1 Empirical Category Confusion Matrix (360 Specimens)"
    elif txt.startswith("C.2 McNemar Contingency Test"):
        p.text = "B.2 McNemar Contingency Test & Normalization Rescues"
    elif txt.startswith("C.3 Non-Parametric Bootstrap"):
        p.text = "B.3 Non-Parametric Bootstrap Confidence Intervals (B = 10,000)"
...
for p in doc.paragraphs:
    ...
    is_h1 = (
        ...
        text_str.startswith("APPENDIX A") or text_str.startswith("APPENDIX B") or
...
# V9 Table of Contents entries: Clean 7 sections + 2 Appendices
toc_entries = [
    ("1. Introduction", "4", 1),
    ("2. Related Work", "5", 1),
    ("3. Methodology", "7", 1),
    ...
    ("7. Conclusion", "21", 1),
    ("ACKNOWLEDGMENT", "22", 1),
    ("APPENDIX A: FIELD SPECIFICATION & OBSERVATION COUNT DERIVATION", "22", 1),
    ("A.1 Document Category Field Structure", "22", 2),
    ("A.2 Mathematical Derivation of 24,480 Paired Observations", "23", 2),
    ("APPENDIX B: EMPIRICAL STATISTICAL METHODOLOGY & BENCHMARKS", "23", 1),
    ("B.1 Empirical Category Confusion Matrix (360 Specimens)", "23", 2),
    ("B.2 McNemar Contingency Test & Normalization Rescues", "24", 2),
    ("B.3 Non-Parametric Bootstrap Confidence Intervals (B = 10,000)", "24", 2),
    ("REFERENCES", "25", 1),
]
...
doc.save(v9_docx_path)
print(f"[SUCCESS] Saved clean PaperV9_Ollama_Primary.docx with Ethics & Appendix A removed & renumbered!")

# Export PDF via Word COM Automation
print(f"Exporting PDF via Word COM Automation: {v9_pdf_path.name}...")
abs_docx = os.path.abspath(str(v9_docx_path))
abs_pdf = os.path.abspath(str(v9_pdf_path))
...
try:
    ...
    doc_word.Close()
    print(f"[SUCCESS] Exported high-quality PDF: {v9_pdf_path.name} ({page_count} Pages!)")
