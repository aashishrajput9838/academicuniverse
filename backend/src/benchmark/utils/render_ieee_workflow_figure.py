"""
IEEE Publication Workflow Figure Generator v4.0 (+5pt Additional Boost -> Total +10pt Boost)
=======================================================================================
Increases every text size by another +5 points (Total +10pt from baseline):
- Title Banner: 19.5 pt -> 24.5 pt
- Start/End Ovals: 15.5 pt -> 20.5 pt
- Main Process Nodes: 14.2 pt -> 19.2 pt
- Decision Diamond: 13.5 pt -> 18.5 pt
- Annotation Callouts: 13.2 pt -> 18.2 pt
...
# 1. Write methodology_workflow.dot with boosted font sizes
dot_content = """digraph G {
    graph [rankdir=TB, nodesep=0.8, ranksep=0.9, fontname="Times-Roman", fontsize=21, compound=true];
    node [shape=box, style="rounded,filled", fillcolor="#F8F9FA", color="#003366", penwidth=2.2, fontname="Times-Roman", fontsize=20.5];
    edge [color="#003366", penwidth=2.2, arrowsize=1.2];
...
# 2. Render 600 DPI PNG, SVG, and Vector PDF via Matplotlib with additional +5pt boost
fig, ax = plt.subplots(figsize=(20, 32), dpi=600)
ax.set_xlim(0, 140)
ax.set_ylim(-20, 235)
ax.axis("off")

# Title Banner (19.5 + 5 = 24.5 pt)
title_box = patches.FancyBboxPatch((3, 218), 134, 13, boxstyle="round,pad=0.5", fc="#003366", ec="#003366", lw=2.2)
ax.add_patch(title_box)
ax.text(70, 224.5, "AU DIC Framework — End-to-End Methodological Workflow", color="white", fontsize=24.5, fontweight="bold", ha="center", va="center", fontfamily="serif")

# Nodes data: (id, label, type, x, y, width, height, fill, border, tc)
# Additional +5pt font sizes applied: Process 19.2pt, Oval 20.5pt, Diamond 18.5pt, Callout 18.2pt
nodes = [
    ("start", "START", "oval", 70, 208, 26, 7.0, "#003366", "#003366", "white"),
    ("config", "Research Configuration", "process", 70, 195, 54, 7.5, "#F8F9FA", "#003366", "black"),
    ("seed", "Deterministic Seed Initialization\n(PrngSeedGenerator)", "process", 70, 181, 60, 8.2, "#F8F9FA", "#003366", "black"),
    ("adbg", "ADBG Synthetic Academic Document Generation", "process", 70, 166, 66, 8.2, "#F8F9FA", "#003366", "black"),
    ("typst", "Typst Vector PDF Compilation", "process", 40, 151, 46, 7.5, "#F8F9FA", "#003366", "black"),
    ("gt_json", "Ground Truth JSON Assembly", "process", 100, 151, 46, 7.5, "#F8F9FA", "#003366", "black"),
    ("raster", "Raster Image Conversion (300 DPI)", "process", 40, 136, 46, 7.5, "#F8F9FA", "#003366", "black"),
    ("degradation", "Controlled Degradation Matrix\n(clean | scanner | camera | rotated)", "diamond", 40, 118, 48, 9.5, "#EDF2F7", "#2B6CB0", "#1A202C"),
    ("dataset", "AU_DIC_Benchmark_v1.0 Dataset Assembly\n(N = 360 Specimen Suite)", "process", 70, 103, 68, 8.2, "#E6FFFA", "#234E52", "#1A202C"),
    ("runner", "Benchmark Runner Engine Initialization\n(isReadOnly: true, allowMockFallback: false)", "process", 70, 89, 68, 8.2, "#F8F9FA", "#003366", "black"),
    ("inference", "Live Vision-Language / OCR Model Inference\n(Groq Cloud Llama 3.1 8B Instant)", "process", 70, 75, 68, 8.2, "#F8F9FA", "#003366", "black"),
    ("prediction", "Raw Prediction Extraction & JSON Parsing", "process", 70, 61, 62, 7.5, "#F8F9FA", "#003366", "black"),
    ("normalizer", "Six-Stage Semantic Canonical Normalization\n(CanonicalNormalizer)", "process", 70, 47, 68, 8.2, "#FEFCBF", "#744210", "#744210"),
    ("comparator", "Field-Level Candidate Comparison", "process", 70, 33, 58, 7.5, "#F8F9FA", "#003366", "black"),
    ("taxonomy", "Nine-Class Structured OCR Error Taxonomy\n(ErrorTaxonomist)", "process", 70, 19, 64, 8.2, "#F8F9FA", "#003366", "black"),
    ("metrics", "Multi-Task Metric Computation", "process", 70, 5, 56, 7.5, "#EBF8FF", "#2B6CB0", "#2B6CB0"),
    ("stats", "Statistical Significance & Uncertainty Analysis", "process", 70, -9, 62, 7.5, "#F0FFF4", "#22543D", "#22543D"),
    ("artifacts", "Publication Artifact Generation", "process", 70, -23, 56, 7.5, "#FAF5FF", "#553C9A", "#553C9A"),
    ("end", "END", "oval", 70, -37, 26, 7.0, "#003366", "#003366", "white"),
]
...
for nid, lbl, ntype, x, y, w, h, bg, border, tc in nodes:
    ...
    if ntype == "oval":
        patch = patches.FancyBboxPatch((x - w/2, y - h/2), w, h, boxstyle="circle,pad=0.4", fc=bg, ec=border, lw=2.2)
        ax.add_patch(patch)
        # Font size boosted to 20.5pt (+5pt more)
        ax.text(x, y, lbl, color=tc, fontsize=20.5, fontweight="bold", ha="center", va="center", fontfamily="serif")
    elif ntype == "process":
        patch = patches.FancyBboxPatch((x - w/2, y - h/2), w, h, boxstyle="round,pad=0.5", fc=bg, ec=border, lw=2.2)
        ax.add_patch(patch)
        # Font size boosted to 19.2pt (+5pt more)
        ax.text(x, y, lbl, color=tc, fontsize=19.2, fontweight="bold", ha="center", va="center", fontfamily="serif")
    elif ntype == "diamond":
        diamond_pts = [[x, y + h/2], [x + w/2, y], [x, y - h/2], [x - w/2, y]]
        patch = patches.Polygon(diamond_pts, closed=True, fc=bg, ec=border, lw=2.2)
        ax.add_patch(patch)
        # Font size boosted to 18.5pt (+5pt more)
        ax.text(x, y, lbl, color=tc, fontsize=18.5, fontweight="bold", ha="center", va="center", fontfamily="serif")

# Callout Annotations boosted to 18.2pt (+5pt more)
callouts = [
    ("normalizer", "Canonical Normalization Callout:\n• ISO Date (YYYY-MM-DD)\n• Hyphen/Space Stripping\n• Case & Whitespace Trim\n• Honorific Prefix Stripper\n• Degree/University Aliases\n• 2-Decimal Float Format", 118, 47, "#FFFBEB", "#D97706"),
    ("metrics", "Computed Metrics Callout:\n• Category Accuracy (%)\n• Precision, Recall, F1\n• Character Error Rate\n• Word Error Rate\n• Joint Exact Match Rate", 118, 5, "#EFF6FF", "#2563EB"),
    ("stats", "Statistical Analysis Callout:\n• McNemar χ² (p < 0.0001)\n• Wilcoxon Signed-Rank\n• Paired t-Test\n• 95% Bootstrap CIs\n• Pass A vs B Ablation", 118, -9, "#ECFDF5", "#059669"),
    ("artifacts", "Publication Callout:\n• metrics.json payload\n• predictions.json logs\n• comparisons.json diffs\n• 600 DPI High-Res Figs\n• Formatted LaTeX Tables", 118, -23, "#F5F3FF", "#7C3AED")
]

for target_id, ctext, cx, cy, cbg, cborder in callouts:
    tx, ty, tw, th = node_dict[target_id]
    cbox = patches.FancyBboxPatch((cx - 17, cy - 5.5), 34, 11, boxstyle="round,pad=0.4", fc=cbg, ec=cborder, lw=1.8, ls="--")
    ax.add_patch(cbox)
    # Font size boosted to 18.2pt (+5pt more)
    ax.text(cx, cy, ctext, color="#1F2937", fontsize=18.2, fontfamily="monospace", ha="center", va="center")
    ax.annotate("", xy=(cx - 17, cy), xytext=(tx + tw/2, ty), arrowprops=dict(arrowstyle="-", color=cborder, linestyle=":", lw=1.8))
...
print("Figure 2 text sizes incremented by another +5pt (Total +10pt) successfully across PNG, SVG, PDF, and DOT!")
