...
def create_omml_display(eq_id):
    """
    Constructs 100% compliant Microsoft Word OMML Display Equations (<m:oMathPara>).
    Guarantees native editable Word Equation objects with right-aligned tags: (1), (2), (3), (4), (5).
    """
    if eq_id == 1:
        xml = f'''
        ...
        <m:oMathPara {nsdecls("m")}>
          <m:oMath>
            ...
            <m:f>
              ...
              <m:den>
                <m:rad><m:radPr><m:degHide m:val="1"/></m:radPr><m:deg/><m:e><m:r><m:t>max( 1, N )</m:t></m:r></m:e></m:rad>
              </m:den>
            </m:f>
            <m:r><m:t> + 3N )</m:t></m:r>
          </m:oMath>
...
def create_omml_inline(text):
    """
    Wraps an inline math symbol into an inline OMML <m:oMath> node with Cambria Math typography.
    """
    clean_txt = (text.replace('$', '')
                     .replace(r'\mu_v', 'μv')
                     .replace(r'\mu', 'μ')
                     .replace(r'\lambda', 'λ')
                     .replace(r'\mathcal{H}', 'H')
                     .replace(r'\epsilon', 'ε')
                     .replace(r'\Delta', 'Δ')
                     .replace(r'\in', '∈')
                     .replace(r'\ge', '≥')
                     ...
                     .replace('t_2', 't₂')
                     .replace('t_{1/2}', 't₁⁄₂')
                     .replace('t_{last}', 't_last')
                     .replace('W_{source}', 'W_source')
                     .replace('V_{total}', 'V_total')
                     .replace('O_{ratio}', 'O_ratio')
                     .replace('D_{avg}', 'D_avg')
                     .replace('K_{complexity}', 'K_complexity')
                     .replace('S_{decayed}', 'S_decayed')
    ...
    xml = f'''
    <m:oMath {nsdecls("m")}>
      <m:r><m:rPr><m:sty m:val="p"/></m:rPr><m:t>{clean_txt}</m:t></m:r>
    </m:oMath>
    '''
    return parse_xml(xml)
...
def clean_prose(text):
    ...
    # Fix spacing defects like "depends onReact"
    text = text.replace('depends onReact', 'depends on React')
    text = text.replace('depends onNode.js', 'depends on Node.js')

    # Standardize math symbols in text
    ...
    # Typography fixes for subscripts/superscripts
    text = text.replace('month^-1', 'month⁻¹')
    text = text.replace('month^{-1}', 'month⁻¹')
    text = text.replace('10^-6', '10⁻⁶')
    text = text.replace('S(t2)', 'S(t₂)')
    text = text.replace('S(t1)', 'S(t₁)')
    ...
    text = text.replace('Wsource', 'W_source')
    text = text.replace('WAU_DIC', 'W_AU_DIC')
    ...
    # Strip inline latex dollar signs and backticks
...
def parse_line_to_paragraph(paragraph, text, default_font_size=11):
    """
    Parses a Markdown line for inline tokens ($math$, **bold**, *italic*, `code`)
    and appends native Word runs or inline OMML math nodes.
    """
...
def build_ieee_production_master():
    ...
    pdf_path = r"c:\github\academicuniverse.com\academicuniverse\Academic_Universe_Paper2_v1.0_IEEE_Production.pdf"
        ...
        for r_idx, row in enumerate(rows):
            ...
            for c_idx, val in enumerate(row):
                if c_idx < len(table.columns):
                    ...
                    parse_line_to_paragraph(p, val, default_font_size=10)
    ...
    while i < len(lines):
        ...
        if line.startswith('```'):
            if in_code_block:
                ...
                if 'GitHub API' in block_content or 'Evidence Intelligence Layer' in block_content:
                    ...
                    parse_line_to_paragraph(p, block_content, default_font_size=9.5)
                code_block_lines = []
            else:
                in_code_block = True
                code_block_lines = []
        ...
        if line.startswith('> '):
            ...
            parse_line_to_paragraph(p, line[2:].strip(), default_font_size=10)
            for r in p.runs:
                r.font.italic = True
        ...
        if line.startswith('# '):
            ...
            parse_line_to_paragraph(p, line[2:], default_font_size=18)
            for r in p.runs:
                r.bold = True
            ...
            p.paragraph_format.keep_with_next = True
            parse_line_to_paragraph(p, line[3:], default_font_size=13)
            for r in p.runs:
                r.bold = True
            ...
            p.paragraph_format.keep_with_next = True
            parse_line_to_paragraph(p, line[4:], default_font_size=11.5)
            for r in p.runs:
                r.bold = True
                r.font.color.rgb = RGBColor(0x0F, 0x17, 0x2A)
        elif line.startswith('#### '):
            ...
            p.paragraph_format.space_after = Pt(2)
            p.paragraph_format.keep_with_next = True
            parse_line_to_paragraph(p, line[5:], default_font_size=11)
            for r in p.runs:
                r.bold = True
                r.italic = True
                r.font.color.rgb = RGBColor(0x33, 0x41, 0x55)
        ...
        # Centered Display Equations
            ...
            omml_elem = create_omml_display(display_eq_counter)
            ...
            if 'Table I:' in line or 'Table II:' in line or '*Table I:' in line or '*Table II:' in line:
                ...
                parse_line_to_paragraph(p_tbl_cap, raw_title, default_font_size=9.5)
            elif line.startswith('*Figure') or line.startswith('Figure') or line.startswith('*Fig.') or line.startswith('Fig.'):
                pass
            else:
                p = doc.add_paragraph()
                parse_line_to_paragraph(p, line, default_font_size=11)
    ...
    doc.save(docx_path)
    print(f"Successfully generated IEEE Production Master DOCX to {docx_path}")

    # Export PDF via Word COM API
    try:
        import win32com.client as win32
        w = win32.Dispatch('Word.Application')
        d = w.Documents.Open(os.path.abspath(docx_path))
        d.SaveAs(os.path.abspath(pdf_path), 17)
        d.Close(0)
        try:
            w.Quit()
        except Exception:
            pass
        print(f"Successfully exported IEEE Production Master PDF to {pdf_path}")
    except Exception as e:
        print(f"PDF export note: {e}")

if __name__ == '__main__':
    build_ieee_production_master()
