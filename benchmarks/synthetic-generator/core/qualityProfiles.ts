...
import { PDFPage, rgb, StandardFonts, degrees, PDFDocument } from 'pdf-lib';
import { QualityProfileConfig, QualityProfileName } from '../types/syntheticGenerator.types';
import { SeededRandom } from './seededRandom';

export const QUALITY_PROFILES: Record<QualityProfileName, QualityProfileConfig> = {
  CLEAN_PDF: {
    name: 'CLEAN_PDF',
...
export class QualityProfileManager {
  ...
  static selectProfile(
    rng: SeededRandom,
    distribution?: Record<QualityProfileName, number>
  ): QualityProfileConfig {
    if (!distribution) {
      // Default distribution: 50% CLEAN, 25% SCANNER, 15% MOBILE, 10% ROTATED
      const rand = rng.nextFloat() * 100;
      if (rand < 50) return QUALITY_PROFILES.CLEAN_PDF;
      if (rand < 75) return QUALITY_PROFILES.SCANNER_COPY;
      if (rand < 90) return QUALITY_PROFILES.MOBILE_CAMERA;
      return QUALITY_PROFILES.ROTATED;
    }
    ...
    for (const [name, pct] of Object.entries(distribution)) {
      cumulative += pct;
      if (rand <= cumulative && name in QUALITY_PROFILES) {
        return QUALITY_PROFILES[name as QualityProfileName];
      }
    }
    return QUALITY_PROFILES.CLEAN_PDF;
  }

  /** Apply quality profile distortions & mandatory research transparency markings */
  static async applyProfileAndDisclaimers(
    pdfDoc: PDFDocument,
    page: PDFPage,
    ...
    page.drawText(
      'SYNTHETIC DATASET — Generated by Academic Universe Research Platform (Fictional Sample For Benchmarking Only)',
      {
        x: 20,
        y: 6,
        size: 7,
    ...
    if (profile.opacityOverlay > 0) {
      const overlayColor = profile.grayscale ? rgb(0.5, 0.5, 0.5) : rgb(0.2, 0.2, 0.2);
      page.drawRectangle({
        x: 0,
        y: 0,
        width,
    ...
    if (profile.rotationDegrees !== 0 && profile.rotationDegrees % 90 === 0) {
      page.setRotation(degrees(profile.rotationDegrees));
    }
  }
}
