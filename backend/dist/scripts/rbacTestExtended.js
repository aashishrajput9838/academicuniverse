...
const getStudentMarks = async (token, studentId) => {
    const res = await (0, node_fetch_1.default)(`${BASE}/api/marks/${studentId}`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
    });
    return res.json().catch(() => ({ success: false, status: res.status }));
};
const updateMarks = async (token, markId) => {
    const res = await (0, node_fetch_1.default)(`${BASE}/api/marks/${markId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
...
const deleteMarks = async (token, markId) => {
    const res = await (0, node_fetch_1.default)(`${BASE}/api/marks/${markId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
    });
...
const run = async () => {
    ...
    for (const u of users) {
        try {
            ...
            // 5) Check /api/auth/me
            const meRes = await (0, node_fetch_1.default)(`${BASE}/api/auth/me`, { headers: { Authorization: `Bearer ${token}` } });
