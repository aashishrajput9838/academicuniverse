...
import { CourseMark, SemesterRecord, StudentProfile } from '../types/syntheticGenerator.types';
...
const COURSES_BY_BRANCH: Record<string, Array<{ code: string; name: string; credits: number }>> = {
  CSE: [
...
export class DataFabricator {
  ...
  generateStudentProfile(): StudentProfile {
  ...
  generateSemesterRecord(semNumber: number, branchCode = 'CSE'): SemesterRecord {
    ...
    const courseMarks: CourseMark[] = coursesToPick.map((c) => {
  ...
  private marksToGrade(marks: number): { grade: string; gradePoint: number } {
