...
class ProjectCache {
  private cache: Map<string, { data: ProjectStats; timestamp: number }> = new Map();
  private readonly TTL = 5 * 60 * 1000; // 5 minutes cache

  set(key: string, data: ProjectStats): void {
    this.cache.set(key, { data, timestamp: Date.now() });
  }

  get(key: string): ProjectStats | null {
...
class GitHubService {
  ...
  private readonly rateLimitPenalty = 30; // Wait X minutes for GitHub API quota renewal

  private isRateLimitBackoffMode = false;
  private backoffDeadline?: Date;
   ...
   */
  async getProjectStats(githubUsername: string): Promise<ProjectStats> {
    // Check if token is configured
    if (!this.accessToken) {
      throw new ConfigurationError('GITHUB_TOKEN environment variable is required. Please configure your GitHub Personal Access Token.');
    }
    ...
    if (this.isRateLimitBackoffMode && this.backoffDeadline && new Date() < this.backoffDeadline) {
      throw new ExternalAPIError('GitHub API rate limit exceeded. Please try again later.');
    }
    ...
    try {
      ...
      const response = await axios.get<GitHubRepository[]>(
        `${this.apiBase}/users/${githubUsername}/repos`,
        {
          headers: {
      ...
      const nonForkRepos = repositories.filter((repo: GitHubRepository) => !repo.fork);
      ...
      if (error.response?.status === 404) {
        throw new ExternalAPIError(`GitHub user '${githubUsername}' not found`);
      }
      
      // Handle other GitHub API errors
      if (error.response) {
        throw new ExternalAPIError(`GitHub API error: ${error.response.status} - ${error.response.statusText}`);
      }
      
      // Handle network errors
      throw new ExternalAPIError('Failed to connect to GitHub API');
    }
  }
   ...
   * @param githubUsername - GitHub username
   */
  clearUserCache(githubUsername: string): void {
    const cacheKey = `github:${githubUsername}`;
  ...
  isRateLimited(): boolean {
    if (!this.isRateLimitBackoffMode) return false;
...
export { ProjectStats, GitHubRepository };