...
import { UniversityTemplateConfig, SyntheticDocumentData } from '../types/syntheticGenerator.types';

export interface ITemplatePlugin {
  config: UniversityTemplateConfig;
  renderDocument(doc: PDFDocument, page: PDFPage, data: SyntheticDocumentData): Promise<void>;
}

export class TemplateEngine {
  private templates: Map<string, ITemplatePlugin> = new Map();

  registerTemplate(template: ITemplatePlugin): void {
    this.templates.set(template.config.id, template);
  }

  getTemplate(templateId: string): ITemplatePlugin {
  ...
  getAllTemplates(): UniversityTemplateConfig[] {
    return Array.from(this.templates.values()).map((t) => t.config);
  }

  private getDefaultTemplate(): ITemplatePlugin {
...
export class DefaultTemplateA implements ITemplatePlugin {
  config: UniversityTemplateConfig = {
    id: 'TEMPLATE_A',
  ...
  async renderDocument(doc: PDFDocument, page: PDFPage, data: SyntheticDocumentData): Promise<void> {
...
export class DefaultTemplateB implements ITemplatePlugin {
  config: UniversityTemplateConfig = {
    id: 'TEMPLATE_B',
...
export class DefaultTemplateC implements ITemplatePlugin {
  config: UniversityTemplateConfig = {
    id: 'TEMPLATE_C',
...
export class DefaultTemplateD implements ITemplatePlugin {
  config: UniversityTemplateConfig = {
    id: 'TEMPLATE_D',
