...
| **Frontend** | Type Safety | TypeScript, global.d.ts | `global.d.ts` | - |
...
| **API** | Rate Limiting | Express rate-limit | `backend/src/middleware/rateLimit.ts` | - |
| **API** | Request Tracking | requestId middleware | `backend/src/middleware/requestId.ts` | - |
...
| **Infrastructure** | Containerization | Docker | `Dockerfile` | - |
| **Infrastructure** | Hosting | Firebase, Vercel | `firebase.json`, `next.config.mjs` | - |
| **Testing** | Unit Tests | Jest | `jest.config.cjs` | - |
| **Testing** | Integration Tests | Jest + Supertest | `backend/src/__tests__/` | - |
| **Monitoring** | Error Tracking | Sentry | `sentry.server.config.ts`, `sentry.edge.config.ts` | - |
| **Logging** | Structured Logging | Winston/Pino | `backend/src/utils/logger.ts` | - |
